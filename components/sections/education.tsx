          ...
          <StaggerChildren className="space-y-12" delay={0.3}>
            {education.map((item, index) => (
              <StaggerItem key={item.id}>
                <div className={`relative ${index % 2 === 0 ? 'md:pr-12 md:text-right md:ml-auto' : 'md:pl-12'} md:w-1/2 md:${index % 2 === 0 ? 'ml-auto' : 'mr-auto'} pl-10 md:pl-0`}>
                  {/* Timeline dot */}
                  <motion.div 
                    className="absolute left-0 md:left-auto md:right-0 top-0 h-6 w-6 rounded-full bg-primary ring-4 ring-background z-10"
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    viewport={{ once: true }}
                    transition={{ delay: 0.2, type: "spring" }}
                    style={{ [index % 2 === 0 ? 'right' : 'left']: '-0.75rem' }}
                  />
